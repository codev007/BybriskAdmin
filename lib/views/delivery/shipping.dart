...
class ShippingDeliveries extends StatefulWidget {
  final String mPincode;
  ShippingDeliveries(this.mPincode, {Key key}) : super(key: key);
  @override
  _ShippingDeliveriesState createState() => _ShippingDeliveriesState();
}

class _ShippingDeliveriesState extends State<ShippingDeliveries> {
  ...
  _fetchDeliveries() async {
    try {
      ...
      final response = await http.post(Constants().shippingDeliveries,
  ...
  void _submit() async {
    try {
      ...
      Map<String, String> headers = {"Content-Type": "application/json"};
      http.Response response = await http.post(Constants().makeOutforDeliver,
          headers: headers, body: json.encode(jsondat));
