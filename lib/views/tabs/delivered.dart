...
import 'package:bybrisk_admin/views/delivery/delivered.dart';
...
class _DeliveredState extends State<Delivered>
    ...
    return InkWell(
      onTap: () {
        Navigator.of(context)
            .push(FadeRouteBuilder(page: DeliveredDetails(overview.pincode)));
      },
      child: Container(
        padding:
  ...
  _fetchDeliveries() async {
    try {
      ...
      final response =
          await http.get(Constants().deliveredOverview, headers: headers);
      if (response.statusCode == 200) {
        setState(() {
          ...
          if (deliveriesList.length > 0) {
            ...
            this._fetchDeliveries();
            BybriskDesign()
                .showInSnackBar("Data not found !", _scaffoldKey);
          }
        });
      }
    } on SocketException {
